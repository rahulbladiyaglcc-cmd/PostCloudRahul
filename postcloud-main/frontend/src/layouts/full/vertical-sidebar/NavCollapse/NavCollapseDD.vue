...
import NavItem from '../NavItem/NavItemDD.vue';
...
<template>
  ...
  <v-list-group no-action>
    ...
    <template v-for="(subitem, i) in item.children" :key="i">
      <NavCollapse :item="subitem" v-if="subitem.children" :level="(props.level ?? 0) + 1" />
      <NavItem :item="subitem" :level="(props.level ?? 0) + 1" v-else></NavItem>
    </template>
  </v-list-group>
